...
{ 
    ...
    while (1) {
		...
		if (count == 5) {
			PC_DispStr(20, 16, "Completed.", DISP_FGND_YELLOW + DISP_BGND_BLUE);
			PC_DispStr(30, 16, "                             ", DISP_FGND_BLACK + DISP_BGND_LIGHT_GRAY);
			strcpy(s, "");
			count = 0;
			err = OSSemPost(Semp);       //每5趟循环发送一次信号量
		} else {
			strcat(s, "..");
			PC_DispStr(30 + count, 16, s, DISP_FGND_YELLOW + DISP_BGND_BLUE);
			count++;
		}
